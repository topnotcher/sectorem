...
class StreamClient:
    ...
    def _sync_subscribe(
        ...
        # Field 0 is always required by Schwab for ADD, SUBS, and VIEW.
        requested_fields = set(fields) | {0}
        ...
        if state is None:
            ...
            fut = self._enqueue_add(service, keys, requested_fields)
            self._wire[service] = _WireState(keys=requested_keys, fields=requested_fields)
            futs.append(fut)
            return futs

        # ADD only adds keys; its fields parameter is ignored for existing subscriptions.
        new_keys = requested_keys - state.keys
        if new_keys:
            log.debug("Existing subscription for %s. Adding keys %s.", service, new_keys)
            fut = self._enqueue_add(service, ",".join(new_keys), {0})
            state.keys |= new_keys
            futs.append(fut)

        # VIEW is the only way to change fields after the initial subscription.
    ...
    def _enqueue_add(self, service: str, keys: str | None, fields: Iterable[int]) -> asyncio.Future[dict]:
        """Enqueue an ADD command for a service."""
        params: dict[str, str] = {"fields": ",".join(str(f) for f in sorted(fields))}
        if keys is not None:
            params["keys"] = keys
